...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name state-machine-operator-builder
	$(CONTAINER_TOOL) buildx use state-machine-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm state-machine-operator-builder
	rm Dockerfile.cross
...
build-config: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > examples/dist/state-machine-operator.yaml
...
test-deploy: manifests kustomize
	...
	$(KUSTOMIZE) build config/default > examples/dist/state-machine-operator-dev.yaml
